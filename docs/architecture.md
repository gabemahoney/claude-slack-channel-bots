...
└── server.ts                   Main entry point — HTTP server, Socket Mode, message routing. Embeds the SR-5.1 startup gate, SR-3.2 template install, SR-1.6 orphan reconcile, SR-2.1 poller. `isHttpVerbose()` (b.3k6) gates the per-request `/mcp` access line behind `CSCB_HTTP_VERBOSE` (truthy: 1/true/yes/on) — off by default, checked per request.
    ...
    ├── health-check.ts         Periodic liveness poller — checks routes on a timer via client.status; schedules restarts for dead sessions; skips channels at the failure cap via `HealthCheckDeps.isAtCap` (SR-25.3/25.4).
...
3. **Liveness check** — `isSessionAlive(channelId)` calls `client.status({claude_instance_id})` and returns true when the state is in `AGENT_DIRECTOR_LIVE_STATES`; `ErrSpawnNotFound` → dead. If alive, `reconnectSession()` calls `reconnectMcp(channelId)` — main returns `'ok'` | `'failed'` | `'dead-session'` (from a `/mcp reconnect slack-channel-router` send-keys with retry), adapter-mapped to `'success'` | `'transient'` | `'escalate-dead'`. A `'success'` reconnect calls `recordSuccess` (resetting the failure count and cap latch); the non-success branches do **not** `recordFailure`. `'escalate-dead'` is currently a **no-op** — nothing re-enters `scheduleRestart` on that outcome, so `restart.ts` simply returns and recovery waits for a future health tick or a server restart. Not counting here keeps the failure count tied to actual launch attempts (the single counting site, SR-25.1) rather than a non-launch reconnect site.
...
1. **Route iteration** — for each `channelId`/`cwd` pair in `routingConfig.routes`:
   ...
   - **Skip if at failure cap** — `isAtCap(channelId, RESTART_FAILURE_CAP)` (injected as `HealthCheckDeps.isAtCap`) returns true; the channel has reached the consecutive-failure cap and is skipped this tick (SR-25.3/25.4). The tick guard stops the *poller* from re-scheduling; the inbound drop branch likewise detects the cap and declines to fire a restart (b.kvq), so a **server restart** is the only path back — it clears the in-process counter and gives the route a fresh attempt.
