...
└── server.ts                   Main entry point — HTTP server, Socket Mode, message routing. Embeds the SR-5.1 startup gate, SR-3.2 template install, SR-1.6 orphan reconcile, SR-2.1 poller.
    ...
    ├── restart.ts              Auto-restart — delayed relaunch on disconnect, exponential backoff (`backoff.ts`), cap-notification (SpawnCapReached once per episode), timer cancellation.
    ├── health-check.ts         Periodic liveness poller — checks routes on a timer via `isSessionAlive` (probe-backed adapter); skips capped channels (`isAtCap`); schedules restarts for dead sessions.
    ├── backoff.ts              Per-channel consecutive-failure counter with exponential backoff. `recordFailure`/`recordSuccess`/`isAtCap`/`shouldNotifyCap`/`nextBackoffDelay` — pure module, no timers/I/O. Cap is 5; base delay from `session_restart_delay`; ceiling 900 s.
    ├── tmux-probe.ts           SR-20.2 `tmux has-session` liveness probe factory (`buildTmuxProbe`, `defaultTmuxProbe`). Injectable exec dep for unit tests. Also exports `classifyAdError` (SR-20.3) and `SEND_KEYS_NOT_FOUND_NEEDLES` (shared with `reconnectMcp`'s ErrTmuxSendKeys discriminator).
    ├── jsonl-persistence-check.ts  Startup safeguard: detects non-persistent JSONL storage (tmpfs/ramfs) by parsing `/proc/self/mountinfo`. `checkJsonlPersistence` / `_runJsonlPersistenceSafeguard` — pure logic with injectable mountinfo reader. Posts a warning to each configured channel when resume is structurally impossible on the host.
    ...
    ├── outage-state.ts         Channel-scoped outage-flag state machine — tracks three orthogonal outage classes (`ad-unreachable`, `tmux-unavailable`, `cwd-unreachable`) per routed channel and emits onset / all-clear Slack posts. Deduplicates same-flag re-raises; emits all-clear only when the flag set empties. `withOutageDetection` / `withSpawnDetection` wrap AD verb calls, mapping `ErrSystemInstallDisappeared`, `ErrTmuxNotAvailable`, `ErrCwdNotFound/ErrCwdNotADirectory` to the appropriate flag. Consumed by `server.ts` (init) and `health-check.ts` (liveness context).
    ├── trust-bootstrap.ts      Pre-accepts Claude Code folder-trust and onboarding dialogs — patches `<claude_config_dir>/.claude.json` for every routed cwd before any spawn fires (startup step 4a-bis). Sets `projects[<cwd>].hasTrustDialogAccepted` and `projects[<cwd>].hasCompletedProjectOnboarding` to `true` via atomic `.tmp` + `renameSync`. Idempotent: skips file write when both flags are already set.
    └── postinstall.ts          npm postinstall script — creates STATE_DIR and MCP config parent directories, writes skeleton `config.json` / `access.json` / `slack-mcp.json` when absent (skips existing files), symlinks bundled skills into `~/.claude/skills/`, migrates legacy `routing.json` → `config.json`, and runs a best-effort `Client.create()` probe to warn early when agent-director is missing (never exits non-zero).
```
...
5. **Per-route reconcile** — `startupSessionManager(routingConfig, { concurrency: 3 })` iterates each configured route concurrently and calls `spawnForRoute` (SR-1.4 collision-then-act dispatcher):
   ...
   - On `ErrInstanceIdCollision`, call `client.get({claude_instance_id})` and branch on state per the [SR-11 substitution table](#sr-11-substitution-table):
     - For any live state (`waiting` / `working` / `pending` / `check_permission` / `ask_user`): run a `tmux has-session` probe first (SR-21.1, always-on). `definitely-dead` → `tryKill` (to move the AD row to terminal) then steer into the `ended`/`missing` resume branch. `transient-inconclusive` → fall through to today's per-state action. `definitely-alive` → fall through to today's per-state action.
     - `ended` / `missing` + `resume_enabled` → `client.resume(...)`; `ErrTmuxSessionCreate` → kill orphan tmux session by deterministic name + `client.spawn(...)` retry once (b.vub self-heal); `ErrNoSessionId` / `ErrJsonlMissing` → `client.delete(...)` + fresh `client.spawn(...)`.
     - `ended` / `missing` + `resume_enabled=false` → `client.kill(...)` + `client.delete(...)` + fresh `client.spawn(...)`.
     - `waiting` (alive) → `reconnectMcp` (sends `/mcp reconnect slack-channel-router` via `client.sendKeys(...)`).
     - `working` (alive) → poll `client.status(...)` until `waiting`, then `reconnectMcp`.
     - `pending` / `check_permission` / `ask_user` (alive) → no-op if alive. If the probe returned `definitely-dead`, these states are steered to the `ended`/`missing` branch and a resume is attempted.
   - `ErrSpawnNotFound` race after collision → single retry-spawn.
   - `ErrTmuxSessionCreate` on the initial fresh-spawn → kill orphan tmux session by deterministic name + single retry-spawn (b.vub self-heal; excluded on post-ladder fresh spawns).
   - Other errors → `postSpawnFailureToChannel(channelId, error)` queues a Slack-channel post (drained after `socket.start()`).
   - **b.uhv / b.k54 / b.vub post-spawn dialog approval:** at every fresh-spawn and resume site in `spawnForRoute`, `approvePreSessionDialogs` is called. This single state-machine-driven function replaces the former two-approver pair. It polls `client.status(...)` and uses pane-first logic: for `pending` rows it drives pane reads and Enter presses via agent-director (`client.readPane` / `client.sendKeys` with `allow_pending: true`); for `missing`/`ended` rows (which arise on the resume path because AD holds the prior terminal state until SessionStart re-fires) it falls back to raw `tmux capture-pane` + `tmux send-keys Enter` keyed on the deterministic session name. Enter is only sent when a known dialog needle (`'Yes, I trust this folder'` or `'I am using this for local development'`) is visible — no stray Enter injected into live sessions. A short no-needle grace streak (`DIALOG_DEAD_GRACE_POLLS`) tolerates the brief post-resume window before the dialog renders, without racing the dialog or re-hanging the resume. Hard cap: 5 minutes; failure is surfaced loudly via `recordStartupError` — never a silent early quit.
...
2. **Failure guard** — exponential backoff delay is computed from the pre-failure count (`nextBackoffDelay`: `min(base × 2^n, 900s)`). After the channel reaches the consecutive-failure cap (`RESTART_FAILURE_CAP` = 5), `onCapReached` fires exactly once per episode (posting a `SpawnCapReached` error to the channel via `postSpawnFailureToChannel`) and no further timers are queued. The cap is cleared by `recordSuccess` (on a successful re-entry from an inbound user message or a server restart).
3. **Timer** — a `setTimeout` fires after the computed backoff delay (base `session_restart_delay` seconds, doubling up to 900 s ceiling).
4. **Liveness check** — `isSessionAlive(channelId)` calls `client.status({claude_instance_id})` and consults a `tmux has-session` probe for live-state rows (SR-21.4): `definitely-dead` → not alive; `transient-inconclusive` → alive (conservative); `definitely-alive` → alive. `ErrSpawnNotFound` → dead. If alive and already reconnected (MCP session in registry), skipped. If alive but disconnected, `reconnectSession()` calls `reconnectMcp(channelId)` which sends `/mcp reconnect slack-channel-router` via `client.sendKeys(...)`; a `'escalate-dead'` result from `reconnectMcp` (dead tmux confirmed by probe) triggers a `scheduleRestart` call to re-enter the dead-session launch path.
...
1. **Route iteration** — for each `channelId`/`cwd` pair in `routingConfig.routes`:
   ...
   - **Skip if at cap** — `isAtCap(channelId)` returns true (the channel has reached `RESTART_FAILURE_CAP` consecutive failures); the tick is skipped but the channel is retry-eligible via an inbound user message or server restart — it is not permanently abandoned (SR-25.3/25.4).
   - **Liveness check** — `isSessionAlive(channelId)` calls `client.status({claude_instance_id})` and for live-state rows also runs a `tmux has-session` probe (SR-21.4): `definitely-dead` → not alive (restart scheduled); `transient-inconclusive` or `definitely-alive` → alive.
2. **Dead session** — if the liveness check reports not alive, `scheduleRestart(channelId, cwd)` is called, delegating to the same restart path used by `onsessionclosed`.
...
2.5. **AD client initialization** — `initClient()` runs the SR-5.1 startup gate (non-exiting `runStartupGate` variant) to install the agent-director Client singleton. `clean_restart` runs in a short-lived CLI process that does not go through the server startup path; without this step `getClient()` would throw. Gate failure exits non-zero with a distinct loud error — never silently falls through.
3. **Per-route precheck** — `directorStatus(channelId)` resolves the actual `claude_instance_id` by label query (survives the b.1m9 naming change). Returns `null` only when no `service=cscb` row exists for the channel (empty list) → skip. AD connectivity errors (connection refused, binary missing, etc.) propagate as rejections to the `Promise.allSettled` pool — they are NOT silently collapsed to null — and the post-loop check exits non-zero if any channel's precheck threw. Terminal states (`ended`, `missing`) → skip; otherwise proceed.
