...
5. **Per-route reconcile** — `startupSessionManager(routingConfig, { concurrency: 3 })` iterates each configured route concurrently and calls `spawnForRoute` (SR-1.4 collision-then-act dispatcher):
   ...
   - On `ErrInstanceIdCollision`, call `client.get({claude_instance_id})` and branch on state per the [SR-11 substitution table](#sr-11-substitution-table):
     ...
     - **b.wrb `ErrJsonlMissing` diagnostic — legible amnesia, unchanged policy.** When resume throws `ErrJsonlMissing` the delete+fresh recovery is unchanged, but `diagnoseJsonlMissing` runs first (before `tryDelete`, while the doomed row's `jsonl_path` / `claude_session_id` / `cwd` / `started_at` are still readable) to make the loss legible. It is a sibling of the b.zak safeguard (step 4a-quater), which runs *before* the resume path; this one runs *inside* it, on the row resume actually failed to open. Never throws. Two things happen:
       ...
       - **never-created vs lost classification.** Using b.zak's read-only `makeDefaultArchiveCount` / `rfc3339ToEpochSeconds` helpers over `message_archive_db`, bounded by `started_at`: **lost** (archive shows ≥1 message since spawn — real conversation history destroyed) is made operator-visible via `recordStartupError('jsonl-transcript-lost-on-resume', …)` (startup only) **and** a `chat.postMessage` to the affected channel, then returns `'lost'`; **never-created** (no archived activity since spawn — the transcript was never written because Claude creates the `.jsonl` lazily on first message; expected and lossless) emits one quiet log line and returns `'never-created'`; **unknown** (row unreadable / AD unreachable before delete) logs what AD reported and returns `'unknown'`. All three outcomes still take the same delete+fresh recovery and all three surface as the `fresh-after-amnesia` action — only `lost` is made loud beyond the log. This is distinct from b.zak's `jsonl-transcript-lost` class (which fires *pre-resume* to warn of an impending wipe); `jsonl-transcript-lost-on-resume` records a wipe that just happened on the row resume tried and failed.
   ...
   - **b.wrb honest startup summary.** `startupSessionManager` tallies each route's `SpawnRouteResult['action']` into a per-outcome breakdown and prints `startupSessionManager: complete — N resumed, N fresh-spawned, N fresh-after-amnesia, N reconnected, N no-op, N failed`. `fresh-after-amnesia` (a fresh spawn that replaced a resume because the transcript was missing) is counted and reported separately, and when it is `> 0` a second loud, grep-friendly line follows pointing at the per-channel `ErrJsonlMissing diagnostic` lines. The returned `StartupSessionManagerResult` carries the same breakdown (`resumed` / `freshSpawned` / `freshAfterAmnesia` / `reconnected` / `noop`) alongside the retained `succeeded` (any non-failed action) and `failed` totals.
