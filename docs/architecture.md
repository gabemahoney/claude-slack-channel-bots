...
└── server.ts                   Main entry point — HTTP server, Socket Mode, message routing. Embeds the SR-5.1 startup gate, SR-3.2 template install, SR-1.6 orphan reconcile, SR-2.1 poller. `isHttpVerbose()` (b.3k6) gates the per-request `/mcp` access line behind `CSCB_HTTP_VERBOSE` (truthy: 1/true/yes/on) — off by default, checked per request.
    ...
    ├── health-check.ts         Periodic liveness poller — checks routes on a timer via client.status; schedules recovery for dead sessions AND for sessions that are alive (AD live state) but MCP-disconnected (b.9a7 — the latter requires `!connected` on two consecutive ticks, a false-positive guard for the freshly-launched window); routes both through `scheduleRestart` (restart.ts then reconnects vs. relaunches per case); skips channels at the failure cap via `HealthCheckDeps.isAtCap` (SR-25.3/25.4), including the new reconnect path.
...
A periodic backstop that runs alongside the reactive disconnect path. It catches two failure modes that the reactive `onsessionclosed` path misses: (a) sessions that die without triggering a close event (e.g., an externally-killed tmux session), and (b) sessions that are alive but MCP-disconnected. Case (b) — an AD live-state row (`waiting` etc.) whose MCP transport is gone — reports `isSessionAlive === true` yet is unreachable. The tick recovers both.
...
1. **Route iteration** — for each `channelId`/`cwd` pair in `routingConfig.routes`:
   ...
   - **Liveness check** — `isSessionAlive(channelId)` calls `client.status({claude_instance_id})`; the state is checked against `AGENT_DIRECTOR_LIVE_STATES`. `isSessionConnected(channelId)` (b.9a7, wraps the same registry-`connected` adapter used by restart.ts) reports whether the MCP session is currently connected.
...
3. **Alive but MCP-disconnected** — if `isSessionAlive` is true but `isSessionConnected` is false, the tick tracks a per-channel consecutive-disconnected streak and calls `scheduleRestart(channelId, cwd)` only once the streak reaches **two consecutive ticks**. The two-tick debounce suppresses the freshly-launched false positive: a booting session that connects between ticks is never poked. Observing the channel as connected (or dead) resets/consumes the streak; `_resetHealthCheckState` clears the whole map. `restart.ts` then reconnects an alive-but-disconnected row (deferring a `working` row per Auto-Restart step 3) or relaunches a dead one.
