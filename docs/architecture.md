...
└── server.ts                   Main entry point — HTTP server, Socket Mode, message routing. Embeds the SR-5.1 startup gate, SR-3.2 template install, SR-1.6 orphan reconcile, SR-2.1 poller. `isHttpVerbose()` (b.3k6) gates the per-request `/mcp` access line behind `CSCB_HTTP_VERBOSE` (truthy: 1/true/yes/on) — off by default, checked per request.
    ...
    ├── restart.ts              Auto-restart — exponential-backoff relaunch on disconnect (SR-25), single-site failure counting, 5-failure cap with once-per-episode `onCapReached`/`ErrSpawnCapReached`, timer cancellation.
...
2. **Backoff timer** — a `setTimeout` fires after `nextBackoffDelay(channelId, session_restart_delay)` seconds: `min(base * 2^preFailureCount, 900)`, so the delay doubles on each consecutive failure from the `session_restart_delay` base up to a 900 s (15 min) ceiling (`src/backoff.ts`, SR-25). The `scheduleRestart` path itself does not consult the cap — it is cap-exempt (the cap only gates the health-check tick, see below). But the only inbound-message trigger for this path is a session that is *registered* for the channel yet lacks a `_GET_stream` (`src/server.ts:711`); a channel capped by 5 consecutive `launchSession` failures has **no** registered session, so an inbound message hits the drop branch (`src/server.ts:631-645`, "No live session for channel … — dropping message") and never calls `scheduleRestart`. Inbound messages therefore recover a registered-but-streamless session but do **not** revive a capped-dead route — and even the streamless-session retry is delayed by the backoff (up to 900 s at the cap with the default base). Only a server restart clears the in-process counter and revives a capped route.
...
6. **Cap latch** — after the 5th consecutive failure (`RESTART_FAILURE_CAP = 5`, exported from `src/restart.ts`), `shouldNotifyCap` fires once and `onCapReached(channelId)` posts a synthetic `ErrSpawnCapReached` to the channel via `postSpawnFailureToChannel`; **no** further timer is scheduled. A capped channel has no registered session, so an inbound Slack message hits the drop branch and does **not** revive it — only a **server restart** clears the in-process counter (lost on restart by design) and gives the route a fresh attempt.
...
1. **Route iteration** — for each `channelId`/`cwd` pair in `routingConfig.routes`:
   ...
   - **Skip if at failure cap** — `isAtCap(channelId, RESTART_FAILURE_CAP)` (injected as `HealthCheckDeps.isAtCap`) returns true; the channel has reached the consecutive-failure cap and is skipped this tick (SR-25.3/25.4). The tick guard stops the *poller* from re-scheduling; because a capped channel has no registered session, an inbound message is dropped rather than triggering recovery, so a **server restart** is the only path back — it clears the in-process counter and gives the route a fresh attempt.
