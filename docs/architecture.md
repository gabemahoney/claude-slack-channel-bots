...
The timer is a self-re-arming `setTimeout` chain aligned to the top of each minute (not `setInterval`), so ticks are serialized — a pass never overlaps the next, which is the property E3's reload seam needs. Each pass, in decision-8 loop order: (1) the E3 reload insertion point (a no-op today); (2) stamp the current wall-clock minute and skip the whole pass unless it is strictly after the last-ticked minute — the **monotonic guard**, so a backwards clock jump cannot re-fire already-fired minutes (matches Vixie cron); (3) match every in-memory schedule against that minute in **server-local time** via croner (DST is croner's, D-Q3); (4) per match, consult the at-most-once `Map<raw line, epoch minute>` — a duplicate is skipped and logged as a loud `SCHEDULER BUG` info line, otherwise the line is recorded and the match collected; (5) prune map entries older than the current minute. All non-duplicate matches for the minute are handed, in crontable line order, to the dispatcher's `fireGroup` in ONE call.

**No catch-up (PD-4):** a tick fires only the minute observed at its start; if a pass overruns the next minute top, that minute is simply skipped — specified behavior, not a bug. A minute with no matches does nothing and logs nothing. The `(raw line, minute)` at-most-once assertion is a **logged bug signal**, not the dispatch mechanism — the minute-aligned single pass already makes a double-fire structurally impossible; the `SCHEDULER BUG` line is an info line kind, deliberately NOT an outcome class, so it never pollutes the `grep http-error` network-failure triage view. There is NO rate cap, backlog cap, busy-detection, session-state check, or queue-depth throttle anywhere in the tick (owner gate answers 1-2).
...
**Same-minute grouping (owner gate answer 3).** When several crontable lines fire in the same minute for the SAME target channel, their freshly-read prompts are concatenated in crontable line order, blank-line separated, and delivered as ONE `/interject` POST. The grouped `sender` then lists every contributor (`cscb-cron:<a>+<b>`, the shared `cscb-cron:` prefix retained on the head and stripped from the tail entries) so no prompt's content is misattributed. Each contributing schedule still gets its own outcome line (carrying a `grouped=N` note) and its own per-fire summary. Grouping is per target channel: same-minute schedules aimed at different channels are never combined. If a combined body would exceed the 32KB `/interject` cap, the group is split back into individual deliveries (one info line records the split) so an oversize concatenation is never silently swallowed; an individually-oversize prompt is skipped and logged `prompt-oversize`, never truncated. Grouping reduces the worst-case POSTs per tick to one per DISTINCT target channel.
