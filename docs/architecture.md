...
└── server.ts                   Main entry point — HTTP server, Socket Mode, message routing. Embeds the SR-5.1 startup gate, SR-3.2 template install, SR-1.6 orphan reconcile, SR-2.1 poller. `isHttpVerbose()` (b.3k6) gates the per-request `/mcp` access line behind `CSCB_HTTP_VERBOSE` (truthy: 1/true/yes/on) — off by default, checked per request.
    ...
    ├── agent-director-errors.ts   SR-0.2 typed Err* re-exports for instanceof branching, plus the CSCB-synthetic `ErrSpawnCapReached` (never emitted by the AD library; minted by the restart cap path — remediation hint "restart the server to retry — automatic restarts are suspended for this channel").
    ...
    ├── restart.ts              Auto-restart — exponential-backoff relaunch on disconnect (SR-25), single-site failure counting, 5-failure cap with once-per-episode `onCapReached`/`ErrSpawnCapReached`, timer cancellation. `scheduleRestart` takes `opts.humanTrigger`, which clamps the computed delay down to `HUMAN_TRIGGER_DELAY_CEILING = 5` s for explicit inbound-message triggers (never up; counter/cap unchanged) (b.kvq).
    ...
    ├── outage-state.ts         Channel-scoped outage-flag state machine — tracks three orthogonal classes (`ad-unreachable`, `cwd-unreachable`, `tmux-unavailable`) per routed channel and emits Slack onset / all-clear messages via an injected `postToChannel`. `server.ts` calls `initOutageState` at boot and `resetAllToHealthy` after `socket.start()`; AD verb calls run inside `withOutageDetection` / `withSpawnDetection`, which raise/clear flags on typed error/success. State mutates before every emit (SR-V-2.x state-before-emit); no timestamps by design. `_resetOutageState` is test-only.
    ...
    ├── cron-dispatch.ts        cscb_cron fire/delivery path — `createCronDispatcher({port, cronLog, cronTablePath})` returns a handle whose `fire(schedule)` delivers ONE matched schedule; it logs every outcome and never throws. The scheduler's tick (`cron-scheduler.ts`) is the only caller — it hands the minute's matches to the dispatcher. The module imports nothing from server.ts, holds no timer, and reads no crontable. Each schedule delivers independently: every match becomes its own `/interject` POST and its own outcome line, so N schedules matching the same minute produce N separate messages. Fixed internal shape (E4's seam, do not collapse): resolve-targets → all-bots deferral branch → target-origin-agnostic per-channel delivery. Targets resolve from the schedule's explicit channel-ID list; the all-bots marker is deferred to E4 — no read, no POST, one `fanout-deferred` outcome (channel `-`) plus a `delivered=0 failed=0` summary (the target set is undefined until E4, and a no-match minute logs nothing, so 0/0 cannot read as quiet success). The prompt file is resolved and fresh-read every fire (never cached): leading `~` via `config.ts` `expandTilde`, absolute as-is, relative anchored to the CRONTABLE's directory — a deliberate divergence from system cron's $HOME convention so a prompt can colocate as `prompts/foo.md` and stay stable (E6's format reference carries this rationale). Missing → `prompt-missing`, unreadable → `prompt-unreadable` (errno), each channel `-`, then a `failed=<targets>` summary and no POST. D-Q5 skip-never-truncate: the oversize predicate is UTF-8 byte length of the EXACT serialized `{channel, message, sender}` body > 32768 (`INTERJECT_BODY_CAP_BYTES`), byte-identical to the /interject handler's own check — the measured string IS the sent string, so a 413 is structurally impossible; over-cap targets log `prompt-oversize` (size, cap) and are skipped, never truncated, never POSTed. Under-cap targets POST the measured body to `http://127.0.0.1:<port>/interject` AS-IS (decision 7 — no extraction, no internal call) with `sender` = schedule identity (D-Q4 — else senderLabel defaults to `interject` and fires are indistinguishable from peer-bot traffic). Status → outcome: 200 `delivered`, 503 `no-session` (decision 1 — NO retry, no queue, no scheduleRestart), 404 `unknown-channel`, else/network error `http-error`. Sequential loop attempts every target; exactly one outcome line per (fire,target) plus one per-fire summary (`failed` = targets whose intended delivery did not happen). Known limitation (recorded, unsolved): a single non-loopback `bind` leaves no loopback listener, so every fire fails connection-refused → `http-error`; no loopback detection is added.
    ├── cron-scheduler.ts       cscb_cron once-per-minute dispatch tick — `createCronScheduler({dispatcher, cronLog, cronTablePath, clock?})` returns a `start`/`stop`/`tick` handle whose single self-re-arming minute-aligned `setTimeout` chain (NOT `setInterval` — ticks are serialized so E3's reload seam mutates the table only between whole passes) is the ONLY code path that dispatches schedules (decision 8). `start()` runs the crontable bootstrap FIRST (this `start()` is the sole `ensureCrontableExists` caller), reads + parses the table EXACTLY ONCE (parse errors → `parse-error` outcome lines, channel `-`), compiles each schedule's croner pattern once (queried, never started — no callback, so it schedules nothing itself), writes exactly ONE `scheduler started, N schedules loaded` info line (PD-4 outage-window marker), then arms the tick; `start()` is failure-isolated and never throws out to the server. Each tick stamps the current wall-clock minute and skips the whole pass unless it is strictly after the last-ticked minute (monotonic guard — a backwards clock jump must not re-fire fired minutes), matches every in-memory schedule against that minute in server-local time via croner (DST is the library's, D-Q3), and hands each match, in crontable line order, to the dispatcher. No catch-up: a tick fires only the minute observed at its start; an overrun minute is simply skipped, and a no-match minute does nothing and logs nothing. The at-most-once `Map<schedule instance, epoch minute>` is a LOGGED ASSERTION, not the mechanism (decision 8): keyed on the compiled-schedule instance (NOT raw line text), so two identical operator lines are distinct schedules that EACH fire (PD-6) — only the SAME loaded schedule reappearing twice in one pass is skipped and shouted as a `SCHEDULER BUG` info line (an info line kind, deliberately NOT an outcome class, so it never pollutes the `grep http-error` triage view). NO rate cap / backlog cap / busy-detection / queue-depth check of any kind (owner gate answers 1-2). `stop()` is idempotent and cancels the pending timer.
    ...
    ├── trust-bootstrap.ts      b.uhv / b.k54 boot-time patcher — `trustBootstrap(routingConfig)` sets `projects[<cwd>].hasTrustDialogAccepted` and `hasCompletedProjectOnboarding` to `true` in each route's `<claude_config_dir>/.claude.json` so the trust-folder dialog never appears at spawn. Idempotent (no rewrite when both flags set); atomic `.tmp` + `renameSync`; never auto-creates a missing file; never throws — per-route failures go to `recordStartupError`. See startup step 4a-bis.
    ...
    ├── jsonl-persistence-check.ts  startup safeguard — detects JSONL transcript loss / non-persistent storage before the resume path; see startup step 4a-quater. Exports `resolveEffectiveConfigDir`, `makeDefaultArchiveCount`, and `rfc3339ToEpochSeconds` — reused by `session-manager.ts`'s b.wrb/b.fwu `diagnoseJsonlMissing` to classify lost / never-created / inconclusive inside the resume path.
    ├── postinstall.ts          npm/bun postinstall scaffold — `runPostinstall()` creates the state dir and MCP config parent, then writes skeleton `config.json`, `access.json` (0o600), and `slack-mcp.json` only when absent (safe to re-run; migrates legacy `routing.json` → `config.json`) and symlinks bundled skills into `~/.claude/skills/`. `runAgentDirectorPostinstallProbe()` best-effort constructs an AD Client and logs one warning on failure — never throws, never fails the install; the SR-5.1 startup gate is the real enforcement.
...
The server fires scheduled prompts from a crontable into bot channels once per minute. The flow crosses four modules — crontable → scheduler tick → dispatcher → the server's own `/interject` → cron log — and is driven entirely by `src/cron-scheduler.ts`'s single minute-aligned timer (decision 8; no per-job timers exist).

Managed sessions learn the crontable path via the `CSCB_CRONTABLE_PATH` export documented in the [Environment Variables](#environment-variables) section.

#### Wiring and startup

The scheduler is constructed and started in `main()` (`server.ts`) **after** `Bun.serve()` is listening, because it is its own `/interject` client — it must POST to a live listener. This is the opposite placement from `startPermissionPoller`, which starts before `Bun.serve`. Wiring is guarded on `routingConfig`: the env-var fallback path constructs nothing cron-related. The dispatcher is handed the server's ACTUAL bound port (`httpServer.port`, covering port-`0` configs), the config-resolved `cron_log_path` and `cron_table_path`. Any construction or `start()` failure is non-fatal — the server keeps serving without a scheduler; `shutdown()` calls `cronScheduler.stop()` alongside `stopHealthCheck`/`stopPermissionPoller`.

`start()` runs, in order: (1) the crontable bootstrap — `ensureCrontableExists`, which this `start()` is the SOLE caller of; a missing crontable is created with the template header and an info line, and the scheduler proceeds with zero schedules; (2) a single read + parse of the table (read and parsed exactly once at start and never re-read), each parse error becoming one `parse-error` outcome line (identity/channel `-`); (3) exactly one `scheduler started, N schedules loaded` info line — the PD-4 outage-window marker, since there is no catch-up, this line is the only bound on a missed-fire window; (4) arming the tick. `start()` is failure-isolated and never throws out to the server.

#### The tick (match → dispatch)

The timer is a self-re-arming `setTimeout` chain aligned to the top of each minute (not `setInterval`), so ticks are serialized — a pass never overlaps the next, which is the property E3's reload seam needs. Each pass, in decision-8 loop order: (1) the E3 reload insertion point (a no-op today); (2) stamp the current wall-clock minute and skip the whole pass unless it is strictly after the last-ticked minute — the **monotonic guard**, so a backwards clock jump cannot re-fire already-fired minutes (matches Vixie cron); (3) match every in-memory schedule against that minute in **server-local time** via croner (DST is croner's, D-Q3); (4) per match, consult the at-most-once `Map<schedule instance, epoch minute>` — keyed on the compiled-schedule instance (identity), NOT raw line text, so two identical operator lines are two distinct schedules that EACH fire (plain cron semantics, PD-6); only the SAME loaded schedule reappearing twice in one pass is skipped and logged as a loud `SCHEDULER BUG` info line, otherwise the instance is recorded and the match collected; (5) prune map entries older than the current minute. All matches for the minute are handed, in crontable line order, to the dispatcher.

**No catch-up (PD-4):** a tick fires only the minute observed at its start; if a pass overruns the next minute top, that minute is simply skipped — specified behavior, not a bug. A minute with no matches does nothing and logs nothing. The `(schedule instance, minute)` at-most-once assertion is a **logged bug signal**, not the dispatch mechanism — the minute-aligned single pass already makes a double-fire structurally impossible; because the key is the compiled-schedule instance rather than raw line text, two identical operator lines are distinct schedules that each fire (PD-6) and never trip the assertion — only the same loaded schedule firing twice in one pass does. The `SCHEDULER BUG` line is an info line kind, deliberately NOT an outcome class, so it never pollutes the `grep http-error` network-failure triage view. There is NO rate cap, backlog cap, busy-detection, session-state check, or queue-depth throttle anywhere in the tick (owner gate answers 1-2).

#### The fire path (dispatcher → /interject → cron log)

For each match the dispatcher (`src/cron-dispatch.ts`) resolves the prompt path and reads the prompt FRESH at fire time (never cached — an edited prompt takes effect next fire), applies the D-Q5 pre-checks (skip, never truncate), and POSTs `{channel, message, sender}` to `http://127.0.0.1:<port>/interject`. `sender` is the schedule identity `cscb-cron:<prompt-file-basename-without-extension>` (D-Q4) — without it `/interject` defaults `senderLabel` to `interject` and a cron fire is indistinguishable from peer-bot traffic. HTTP status maps to outcome class: 200 → `delivered`, 503 → `no-session` (decision 1 — NO retry, no queue, no `scheduleRestart`), 404 → `unknown-channel`, any other status or a network failure → `http-error`. Every outcome plus one per-fire summary (`delivered`/`failed` counts) is appended to the cron log. A channel-less (all-bots) line is matched but logged `fanout-deferred` and NOT dispatched — no read, no POST — with a `delivered=0 failed=0` summary; fan-out is not yet enabled.

**Same-minute matches deliver independently.** Each matched schedule is delivered on its own — its own freshly-read prompt, its own `/interject` POST with `sender` = its schedule identity, and its own outcome line and per-fire summary. When several crontable lines match the same minute for the same channel, each one arrives as its own message; they are not combined.

The cron-log outcome classes are defined once in [cron.log](#cronlog-cron_log_path) — the taxonomy there is authoritative; this subsection does not restate the class names.

**Bind constraint.** The scheduler POSTs to `127.0.0.1`, so a `bind` on a single non-loopback interface leaves no loopback listener and every fire fails connection-refused → `http-error`. `bind` must include loopback (the default `127.0.0.1`, or `0.0.0.0`) for scheduled fires to work. This is log-visible and accepted; no loopback detection is added.
...
4a-quater. **b.zak JSONL-persistence safeguard** — `runJsonlPersistenceSafeguard(routingConfig, web)` (from `src/jsonl-persistence-check.ts`). Call site: `main()` in `server.ts`, immediately after step 4a-bis (`trustBootstrap`) and before step 4a-ter (`stopHookBootstrap`) — critically, **before** step 5's resume path, which silently `delete+fresh`-spawns a memory-less bot when `resume` throws `ErrJsonlMissing`. Detection/reporting only: it never changes resume behavior, never throws, and never blocks startup (wrapped in its own `try/catch` at the call site as well). Dry-run passes `undefined` web to suppress Slack posts. Sibling scope: the lost / never-created / inconclusive classification inside the resume path itself is **b.wrb / b.fwu**, not this module. Two layers:

    - **Layer 1 — non-persistent storage.** `checkJsonlPersistence` resolves the deduped set of `<effectiveConfigDir>/projects` roots (`route.claude_config_dir ?? routingConfig.claude_config_dir`, homedir default) and classifies each root's mount fstype by longest-prefix match over `/proc/self/mountinfo` (string-prefix, not a stat — a not-yet-created dir classifies by its parent mount). A root on `tmpfs`/`ramfs` → `recordStartupError('jsonl-non-persistent', …)` plus one `chat.postMessage` warning to the affected channels — those whose effective JSONL root is the flagged root, not every routed channel (resume is structurally impossible for those channels on this host). An undeterminable fstype (unreadable/unparseable/unresolvable) → `recordStartupError('jsonl-persistence-check-warning', …)`, no Slack post.
    - **Layer 2 — per-channel transcript loss.** For each route it fetches the AD row (through `withOutageDetection`, the sanctioned CSCB→AD entry point) and, when the row carries a `claude_session_id`, stats the AD-recorded `jsonl_path` **and** a config-dir-aware fallback path composed the same way agent-director composes it (`resolveJsonlPath`). Outcomes: persisted exists → healthy, silent. Persisted stale/missing but fallback exists → `recordStartupError('jsonl-transcript-stale-path', …)` + a Slack post to that channel (history is on disk but AD 0.8.0's resume only stats the persisted path, so it would throw `ErrJsonlMissing` and wipe the channel). Both missing but the `message_archive_db` archive (when configured) holds messages in that channel since `started_at` → `recordStartupError('jsonl-transcript-lost', …)` + a Slack post (conversation provably happened, transcript is gone). The archive check is read-only: a missing archive DB file yields no evidence (it is never created here). Both missing with no archive evidence → a single quiet log line only: an idle-since-spawn channel has no transcript yet because Claude creates the `.jsonl` lazily on first message — normal, not an error, and must not alarm every boot. Per-channel AD/get errors (including `ErrSpawnNotFound`) are logged and skipped; the sweep never aborts.
4a-ter. **b.osj stop-hook bootstrap** — `stopHookBootstrap(routingConfig)` (from `src/stop-hook-bootstrap.ts`) patches `<claude_config_dir>/settings.json` for every routed effective config dir to install (or remove) the CSCB-managed Slack Reply Guard Stop hook. Call site: `main()` in `server.ts`, immediately after step 4a-quater (`runJsonlPersistenceSafeguard`) and before step 4b (`reconcileInstanceIds`) — before any spawn fires. Runs in both real and dry-run modes (config-file patch, not a session operation). Never throws to the caller; per-dir failures are caught and recorded via `recordStartupError`. Behavior:
...
5. **Per-route reconcile** — `startupSessionManager(routingConfig, { concurrency: 3 })` iterates each configured route concurrently and calls `spawnForRoute` (SR-1.4 collision-then-act dispatcher):
   - Attempt `client.spawn(SR-1.1 params)` — `template: 'slack-channel-bot'`, `relay_mode: 'on'`, `label: ['service=cscb', 'channel=<id>']`, `claude_instance_id: cscb_<normalizedName>_<channelId>` (falls back to `cscb_<channelId>` when name resolution failed), `tmux_session_name: slack_bot_<normalizedName>_<channelId>` (same fallback), optional `extra_env.CLAUDE_CONFIG_DIR`, and `extra_env.CSCB_CRONTABLE_PATH` — the latter present on EVERY route (unconditional, unlike the optional `CLAUDE_CONFIG_DIR`), carrying the resolved tilde-expanded `cron_table_path` (the default `<config dir>/crontab` or the configured override).
   - On `ErrInstanceIdCollision`, call `client.get({claude_instance_id})` and branch on state per the [SR-11 substitution table](#sr-11-substitution-table):
     - `ended` / `missing` + `resume_enabled` → `client.resume(...)`; `ErrNoSessionId` / `ErrJsonlMissing` → `client.delete(...)` + fresh `client.spawn(...)`; `ErrSpawnNotFound` (row vanished between the dead-session verdict and resume — operator delete, expire, race) → fresh `client.spawn(...)` directly with no delete (the row is already gone), action `spawned`. On `ErrJsonlMissing` specifically, `diagnoseJsonlMissing` runs **before** the delete (see the b.wrb / b.fwu note below) — the delete+fresh policy itself is unchanged — and the resulting fresh spawn returns action `fresh-after-amnesia` (definite lost/never-created verdict) or `fresh-after-inconclusive-amnesia` (b.fwu — diagnosis inconclusive) rather than `spawned`, so the startup summary never counts a memory-loss recovery as an ordinary clean spawn.
     ...
     - **b.m4r shared memoized sweep — `reconcileMissingSweep`.** Three sweep callers go through one shared `reconcileMissingSweep` helper rather than calling `client.findMissing({})` inline: this `working`-row up-front sweep, `resumeOrFreshSpawn`'s `reconcileMissingFirst`, and the tick/restart escalate-dead path (via the exported `sweepDeadTmuxChannel` wrapper, which prepends its unconditional operator log then delegates here — `reconcileMissingSweep` itself stays module-private). Because `findMissing({})` is a whole-store, idempotent sweep, a fleet restart — `startupSessionManager` runs per-channel at `concurrency=3`, and every `working`-row collision plus each dead-path `reconcileMissingFirst` would otherwise fire its own whole-store sweep — is collapsed by single-flight (concurrent callers share one in-flight sweep promise) plus a 10s TTL memo (a caller arriving just after the last successful sweep reuses that result). This sheds the redundant N-sweep amplification without changing recovery behavior; failures are never memoized, so the next caller after an error re-sweeps. The post-reboot fleet-wide case (b.nk5 — `/tmp` wiped, every channel dead-tmux at once) is served correctly: one in-flight-shared sweep reconciles the whole store for every simultaneously-escalating channel. The 10s memo TTL is harmless against the ~120s health-check tick cadence: a memoized-stale answer on the escalate-dead path costs at most **one** extra tick, because the following tick's escalate-dead sweeps again well past the TTL. (The `reconcileMissingFirst`/`working`-row callers are startup-clustered within one boot rather than tick-spaced, so they benefit from the memo directly.)
     - `pending` / `check_permission` / `ask_user` → no-op (poller picks it up).
     - **b.wrb `ErrJsonlMissing` diagnostic — legible amnesia, unchanged policy.** When resume throws `ErrJsonlMissing` the delete+fresh recovery is unchanged, but `diagnoseJsonlMissing` runs first (before `tryDelete`, while the doomed row's `jsonl_path` / `claude_session_id` / `cwd` / `started_at` are still readable) to make the loss legible. It is a sibling of the b.zak safeguard (step 4a-quater), which runs *before* the resume path; this one runs *inside* it, on the row resume actually failed to open. Never throws. Two things happen:
       - **Path + provenance logging.** If AD's error description enumerates every transcript candidate it tried as `<source> <path> (<stat error>)` (a future AD release, bug b.1ba — installed 0.8.0 does **not** carry this), those are parsed and logged verbatim, labelled *paths+sources reported by agent-director*. Otherwise (0.8.0) CSCB reconstructs the candidates locally — the persisted `jsonl_path` column plus a `CLAUDE_CONFIG_DIR`-aware fallback via `resolveJsonlPath` and b.zak's `resolveEffectiveConfigDir` — and labels them explicitly *locally computed* so a reader never mistakes a CSCB-side reconstruction for what AD actually stat'd. The parse keys off the literal `persisted`/`fallback` tokens, not any version string, so it degrades cleanly.
       - **lost / never-created / inconclusive classification (b.fwu).** Using b.zak's read-only `makeDefaultArchiveCount` / `rfc3339ToEpochSeconds` helpers over `message_archive_db`, bounded by `started_at`, `diagnoseJsonlMissing` returns one of `'lost' | 'never-created' | 'inconclusive'`:
         - **lost** — archive shows ≥1 message since spawn, real conversation history destroyed. Made operator-visible via `recordStartupError('jsonl-transcript-lost-on-resume', …)` (startup only) **and** a `chat.postMessage` to the affected channel.
         - **never-created** — strictly evidence-based: the archive was actually consulted and returned 0 messages since spawn, so the transcript was never written (Claude creates the `.jsonl` lazily on first message; expected and lossless). One quiet log line only.
         - **inconclusive (b.fwu)** — "no evidence either way": the AD row fetch failed, `started_at` was unparseable, the archive read failed, or `message_archive_db` is simply not configured. Previously these fell through to a reassuring `never-created`/console-only `unknown`, silently downgrading a possible real loss. Now each emits `recordStartupError('jsonl-diagnosis-inconclusive', …)` carrying the reason (row-fetch failed / started_at unparseable / archive unconfigured / archive read failed) **and** a `chat.postMessage` worded as *uncertainty* ("restarted fresh; could not determine whether prior history was preserved") — deliberately distinct from the `lost` wording, since a false "your history was destroyed" is its own harm. When `message_archive_db` is unconfigured the operator message states diagnosis is impossible because no message archive is configured, with a hint to enable it.

         All three outcomes still take the same delete+fresh recovery (policy unchanged). `lost` and `never-created` surface as the `fresh-after-amnesia` action; **inconclusive** surfaces as the new `'fresh-after-inconclusive-amnesia'` action (see the honest-summary note below). This is distinct from b.zak's `jsonl-transcript-lost` class (which fires *pre-resume* to warn of an impending wipe); `jsonl-transcript-lost-on-resume` records a wipe that just happened on the row resume tried and failed.
   ...
   - **b.wrb / b.fwu honest startup summary.** `startupSessionManager` tallies each route's `SpawnRouteResult['action']` into a per-outcome breakdown and prints `startupSessionManager: complete — N resumed, N fresh-spawned, N fresh-after-amnesia, N fresh-after-inconclusive-amnesia, N reconnected, N no-op, N failed`. `fresh-after-amnesia` (a fresh spawn replacing a resume where the archive gave a definite lost/never-created verdict) and `fresh-after-inconclusive-amnesia` (b.fwu — the same recovery but the diagnosis was inconclusive, so it is *unknown* whether history was preserved) are counted and reported separately; each has its own loud, grep-friendly follow-up line when its count is `> 0`, pointing at the per-channel `ErrJsonlMissing diagnostic` lines. The returned `StartupSessionManagerResult` carries the same breakdown (`resumed` / `freshSpawned` / `freshAfterAmnesia` / `freshAfterInconclusiveAmnesia` / `reconnected` / `noop`) alongside the retained `succeeded` (any non-failed action) and `failed` totals.
...
2. **Backoff timer** — a `setTimeout` fires after `nextBackoffDelay(channelId, session_restart_delay)` seconds: `min(base * 2^preFailureCount, 900)`, so the delay doubles on each consecutive failure from the `session_restart_delay` base up to a 900 s (15 min) ceiling (`src/backoff.ts`, SR-25). `scheduleRestart` accepts `opts.humanTrigger`; when set, the computed delay is clamped **down** to `HUMAN_TRIGGER_DELAY_CEILING = 5` s (never raised) so a person typing in the channel isn't told to wait out a 900 s backoff. The clamp only shortens the wait — it does not reset the failure counter and does not bypass the re-entrancy guard, so each attempt still counts toward normal backoff/cap accounting. The `scheduleRestart` path itself does not consult the cap — it is cap-exempt (the cap only gates the health-check tick and the inbound message triggers, which check `backoffIsAtCap` themselves before firing, see below). Two triggers reach this path from inbound messages: a session that is *registered* for the channel yet lacks a `_GET_stream` (the streamless branch of `handleMessage`, whose `humanTrigger` `scheduleRestart(ownerChannelId, targetSession.cwd, undefined, { humanTrigger: true })` fires only when the route is live-but-streamless, under the cap, auto-restart-enabled, and not already restarting — b.9cj: this branch no longer calls `notification()` afterward, since the SDK's `send()` would evaporate silently with the stream gone; it returns after triggering recovery and posts the sender an honest per-case reply, matching the b.kvq drop-branch reply shape), and — as of b.kvq — a configured channel with **no** registered session (the "No live session … — dropping message" drop branch in `handleMessage`, its `humanTrigger` `scheduleRestart`), which likewise fires unless a restart is already pending/active, auto-restart is disabled (`session_restart_delay: 0`), or the route is at the failure cap. Both branches key every guard and the `scheduleRestart` call on the **owning** channel (`ownerChannelId` — for a direct route that is the inbound channel; for a `default_route` channel or DM it is the direct route whose `cwd === default_route`, resolved via `targetSession.channelId` in the streamless branch), so a `default_route` channel's recovery never spawns a duplicate instance under the inbound channelId. At the cap the inbound path does **not** bypass the cap; only a server restart clears the in-process counter and revives a capped route. In all cases the dropped message itself is lost — recovery starts a session but never delivers or replays that message.
3. **Liveness check** — `isSessionAlive(channelId)` calls `client.status({claude_instance_id})` and returns true when the state is in `AGENT_DIRECTOR_LIVE_STATES`; `ErrSpawnNotFound` → dead. When alive, `restart.ts` skips the reconnect as "already reconnected" only when the session is connected **AND** its standalone GET SSE stream is present (`RestartDeps.hasSessionStream`, b.9cj); a connected-but-streamless session (`connected === true` but the SDK dropped `_GET_stream`) cannot receive messages, so it is *not* waved through — it proceeds to recovery. If alive and not skipped, `reconnectSession()` first probes AD state and, when the row is `working`, returns `'transient'` without poking the pane — deferring the `/mcp reconnect` send-keys to a later tick so a mid-long-turn session is not corrupted (b.9a7 hazard 2, preserving the b.rmy invariant: a `working` row is never declared dead, only left alone; on a status error it falls through to the reconnect attempt rather than manufacture a false defer). Otherwise `reconnectMcp(channelId)` runs — main returns `'ok'` | `'failed'` | `'dead-session'` (from a `/mcp reconnect slack-channel-router` send-keys with retry), adapter-mapped to `'success'` | `'transient'` | `'escalate-dead'`. A `'success'` reconnect calls `recordSuccess` (resetting the failure count and cap latch); the non-success branches do **not** `recordFailure`. On `'escalate-dead'` / `'transient'` (including the `working` defer) `restart.ts` does not re-enter `scheduleRestart`; instead the **next health-check tick** re-observes the channel and, if it is still not deliverable (alive-but-disconnected or connected-but-streamless) or has since gone dead, calls `scheduleRestart` again, so a failed/deferred reconnect is retried on a bounded cadence. For the dead-tmux `escalate-dead` case CSCB recovers itself internally: the `_buildReconnectSessionAdapter` mapping awaits `sweepDeadTmuxChannel(channelId, verdict)` (`src/session-manager.ts`) before returning the verdict. That wrapper emits an operator-visible log line (`[slack] escalate-dead: channel=… verdict=… — tmux session provably dead, triggering internal findMissing reconciliation …`) then runs the memoized `reconcileMissingSweep` (b.m4r), which reconciles the frozen `working` row to `missing`; the next tick then observes `alive === false` and takes the normal kill+relaunch branch. The external `~/startup/find-missing-loop.sh` is now belt-and-braces only — recovery no longer depends on it, and removing it stays a separate operator decision. Not counting here keeps the failure count tied to actual launch attempts (the single counting site, SR-25.1) rather than a non-launch reconnect site — the sweep records neither success nor failure.
4. **Kill zombie** — `client.kill({claude_instance_id})` is best-effort; `ErrSpawnNotFound` is ignored.
5. **Relaunch** — `launchSession(channelId, cwd, routingConfig)` collapses `spawnForRoute`'s richer outcome to a boolean — the single counting site (SR-25.1). Resume vs fresh is decided by the SR-1.4 collision-then-act dispatch (no separate `sessionId` arg from CSCB — agent-director owns resume state). On a `true` outcome, `recordSuccess` resets the counter and cap latch; on `false`, `recordFailure` increments it.
6. **Cap latch** — after the 5th consecutive failure (`RESTART_FAILURE_CAP = 5`, exported from `src/restart.ts`), `shouldNotifyCap` fires once and `onCapReached(channelId)` posts a synthetic `ErrSpawnCapReached` to the channel via `postSpawnFailureToChannel`; **no** further timer is scheduled. A capped channel has no registered session; an inbound Slack message hits the drop branch, which detects the cap (`backoffIsAtCap`) and does **not** fire a restart — firing one would only burn a spawn attempt against a route that cannot come up. The sender is told the channel will not self-recover. Only a **server restart** clears the in-process counter (lost on restart by design) and gives the route a fresh attempt.

### Health-Check Poller

A periodic backstop that runs alongside the reactive disconnect path. It catches three failure modes that the reactive `onsessionclosed` path misses: (a) sessions that die without triggering a close event (e.g., an externally-killed tmux session); (b) sessions that are alive but MCP-disconnected — an AD live-state row (`waiting` etc.) whose MCP transport is gone, `isSessionAlive === true` yet unreachable; and (c) connected-but-streamless sessions (b.9cj) — `isSessionAlive === true` and `isSessionConnected === true` in the registry, but the SDK has silently dropped the `_GET_stream` map entry, so messages cannot reach the bot. Without (c) an idle streamless channel would stay broken forever, invisible to the reactive path. The tick recovers all three.
...
1. **Route iteration** — for each `channelId`/`cwd` pair in `routingConfig.routes`:
   ...
   - **Skip if at failure cap** — `isAtCap(channelId, RESTART_FAILURE_CAP)` (injected as `HealthCheckDeps.isAtCap`) returns true; a channel at the consecutive-failure cap is skipped this tick (SR-25.3/25.4), including the alive-but-disconnected reconnect path — a capped channel gets no tick-driven reconnect. The inbound drop branch likewise detects the cap and declines to fire a restart, so a **server restart** is the only path back — it clears the in-process counter and gives the route a fresh attempt.
   - **Liveness check** — `isSessionAlive(channelId)` calls `client.status({claude_instance_id})`; the state is checked against `AGENT_DIRECTOR_LIVE_STATES`. `isSessionConnected(channelId)` (b.9a7, wraps the same registry-`connected` adapter used by restart.ts) reports whether the MCP session is currently connected. `hasSessionStream(channelId)` (b.9cj, wraps the shared `getSessionByChannel` → `hasGetStreamKey(transport)` probe wired into both `HealthCheckDeps` and `RestartDeps`) reports whether the standalone `_GET_stream` is present.
...
3. **Alive but not deliverable** — if `isSessionAlive` is true but the session is either not connected (`!isSessionConnected`, b.9a7) **or** connected-but-streamless (`!hasSessionStream`, b.9cj), the tick tracks a per-channel consecutive-failing streak and calls `scheduleRestart(channelId, cwd)` only once the streak reaches **two consecutive ticks**. Both cases land in this branch and share the one streak/debounce, which suppresses the freshly-launched false positive: a booting session that connects (and re-opens its stream) between ticks is never poked. Observing the channel as fully healthy — alive, connected, AND stream present — (or dead) resets/consumes the streak; `_resetHealthCheckState` clears the whole map. `restart.ts` then reconnects an alive-but-disconnected row (deferring a `working` row per Auto-Restart step 3), recovers a streamless one, or relaunches a dead one. This gives time-bounded recovery on the `health_check_interval` (default 120 s) even for idle channels that receive no inbound traffic.
...
3. **Init agent-director client** (b.qwo) — this short-lived CLI process does not run the server startup gate, so `getClient()` would otherwise throw. `initClient` (`CliDeps`) installs the Client singleton via the non-exiting `runStartupGate` variant. A gate failure is a distinct loud `exit(1)` — the restart never proceeds to teardown or `start` on top of bots it could not reach. Skipping this init was the b.qps / `incident-2026-09-18-clean_restart.md` root cause: the throw was swallowed and every route reported a bogus "no spawn row — skipping".
4. **Per-route precheck** — `directorStatus(channelId)` returns `null` only when the row is genuinely absent (empty label-list result); "no spawn row — skipping" is printed only in that case. Terminal states (`ended`, `missing`) → skip; otherwise proceed. Any error reaching agent-director (connection refused, uninitialized client, binary unreachable) **propagates** — it is no longer collapsed into a `null`/"no row". Such rejections surface after the parallel loop as a loud aggregate error (see step 6).
5. **Pause + poll** — `directorPause(channelId)` is called; if it rejects, escalate immediately to `directorKill(channelId)`. After a successful pause, `directorStatus(channelId)` is polled with exponential backoff (100 ms start, capped at 2 s, total budget `exit_timeout`). The first poll observing `null` / `ended` / `missing` is treated as clean exit. If the budget is exhausted, `directorKill(channelId)` force-terminates the spawn. On both the escalation and timeout kill paths the kill's outcome decides the channel's fate (b.dnt): a kill failure other than the benign already-gone race (`ErrSpawnNotFound`) is itself a teardown failure surfaced as a rejection in step 6. Teardown is **kill-but-never-delete**: rows are paused/killed but never deleted, preserving each row's `claude_session_id` so the next start can resume with context.
6. **Loud AD-unreachable failure** (b.qwo, b.dnt) — after the parallel loop, any settled rejection (an agent-director connectivity error raised by a precheck in step 4, a poll-loop `directorStatus` in step 5, or (b.dnt) an escalation- or timeout-path `directorKill` that failed with anything other than the benign already-gone race, never a normal terminal-row skip and never an `ErrSpawnNotFound` already-gone kill, both of which resolve quietly) is logged and re-thrown as an aggregate error. `clean_restart` catches it and `exit(1)`, aborting the restart **before** starting a new server. A silent no-op teardown is thus impossible.
7. **Start new server daemon** — shells out to `claude-slack-channel-bots start`. Reached only if teardown completed without an AD-unreachable failure.
8. **Exit** — a non-zero exit code from `start` is propagated and the process exits with that code.
...
| `claude-director find-missing` | `client.findMissing({})` — driven through the shared memoized `reconcileMissingSweep` (b.m4r) from the dead-session pre-resume path (b.4dk, `reconcileMissingFirst`) and, via `sweepDeadTmuxChannel`, from the tick/restart escalate-dead path (dead-tmux self-recovery). Also runnable by operators as a periodic sweep (SR-5.4). |
...
- `cron_table_path` — optional path to the cscb_cron crontable file (default: `<config dir>/crontab`, where `<config dir>` is the directory of the loaded config file). Registered in `KNOWN_TOP_LEVEL_KEYS`; `validateConfig` rejects anything that is not a non-empty string; `~` is expanded and the path is resolved to absolute, yielding a required string on the resolved config. Boot-fixed: only the crontable's contents hot-reload (a later Epic); its location never does.
- `cron_log_path` — optional path to the cscb_cron log file (default: `<config dir>/cron.log`). Same registration, non-empty-string validation, tilde expansion, required-resolved-string, and boot-fixed notes as `cron_table_path`.
...
**No-truncation guarantee.** The canonical trail line persists every field at full fidelity (SR-V-3.1) — the full `blocks` array including every `action_id`, the full `text`, the full Slack API response. The 300-character `console.error('[slack] RAW … event:', JSON.stringify(...).slice(0, 300))` summaries in the socket `message` / `app_mention` handlers (near `src/server.ts:804` / `:817`) are a **separate human-readable log** kept truncated intentionally per SR-V-3.2; they do not replace the canonical store and the two coexist.
...
**Note on the socket `message` / `app_mention` truncation (near `src/server.ts:804` / `:817`).** The existing `console.error('[slack] RAW … event:', JSON.stringify(...).slice(0, 300))` summaries are INTENTIONALLY preserved per SR-V-3.2 — they are a separate human-readable log. The canonical store is full-fidelity by construction (no `.slice` in the trail emit path).
...
CSCB_CRONTABLE_PATH is set at spawn time via SpawnParams.extra_env for every managed session — unconditionally, on every route — and is propagated by agent-director to the spawn's tmux session. It carries the resolved tilde-expanded `cron_table_path` (the default `<config dir>/crontab` or the configured override) so bots can self-schedule by locating and appending to the crontable, the single source of truth for scheduled prompts (bots are first-class writers alongside the server).
