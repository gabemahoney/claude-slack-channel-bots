...
`start()` runs, in order: (1) the crontable bootstrap — `ensureCrontableExists` — which this `start()` is now the SOLE caller of (server wiring removed its own call per PM review); a missing crontable is created with the template header and an info line, and the scheduler proceeds with zero schedules; (2) a single read + parse of the table (E3 adds hot-reload; today the table is loaded once and never re-read), each parse error becoming one `parse-error` outcome line (identity/channel `-`); (3) exactly one `scheduler started, N schedules loaded` info line — the PD-4 outage-window marker, since there is no catch-up, this line is the only bound on a missed-fire window; (4) arming the tick. `start()` is failure-isolated and never throws out to the server.
