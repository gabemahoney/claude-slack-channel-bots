...
- The streamless-session trigger (the human-clamped `scheduleRestart(ownerChannelId, targetSession.cwd, undefined, { humanTrigger: true })` in the streamless branch of `handleMessage` in `src/server.ts`) fires for a session that is *registered* yet missing its `_GET_stream`. A capped-dead route has no such registered session, so this never applies to it. As of b.9cj this branch does **not** fall through to `notification()` afterward — with the stream gone the SDK's `send()` evaporates silently, so the message is provably undeliverable; the branch returns after triggering recovery and posts the sender an honest per-case reply instead of silence. It mirrors the b.kvq drop-branch guard shape: it resolves the **owning** channel (`ownerChannelId = targetSession.channelId`, since `targetSession` may have been reached via `default_route`/`getSessionByCwd` or the DM path, so the inbound `channelId` is not always the owner) and keys the `isRestartPendingOrActive` / `backoffIsAtCap` guards and the `scheduleRestart` call on it — never the inbound channel — while the reply is posted back to the inbound channel. `scheduleRestart` fires only in the "live, streamless, under cap, auto-restart enabled" case; when a restart is already pending/active, auto-restart is disabled, or the route is capped it does **not** fire and the reply says so. Recovery now actually fires because `restart.ts` no longer waves a connected-but-streamless session through as "already reconnected" (it skips only when connected AND `hasSessionStream`).
...
- **Resolved outside the repo.** The fix is ops-only or lives in another file/host — name the file/host changed (e.g. `~/startup/start-all.sh`, `~/.claude/channels/slack/system-prompt.md`). Keep the `~/` path and the resolution claim connected (same line or same closure section); a `~/` path merely quoted in unrelated evidence does not count as a closure.
...
- **Closed without a code change.** It is explicitly no-repro, superseded, abandoned, or satisfied by other work — say which, and point at where the real work lives (or why none is needed). The audit recognizes reasoned phrasings such as "no-repro" / "not reproducible" / "cannot reproduce" / "wont-fix", "closed as superseded/abandoned", and the satisfaction idioms "superseded by", "fully satisfied by", "already satisfied". A bare, reasonless `## Closed` heading does **not** count — a closure needs a stated reason.
...
> **SR-2.6 is known-red today — by design, not a gate defect.** The stranded-work audit currently exits non-zero because one class of hygiene debt remains. Stranded finished tickets are clean (none). The sole remaining finding is a stranded remote branch whose resolution is push-class (archive, push, delete the remote ref, `git fetch --prune`) and is therefore reserved for the repo owner — currently `origin/no-channels`, tracked on ticket **`b.gkz`** pending the owner's go-ahead. A non-zero SR-2.6 is the correct, expected result until `b.gkz` lands; it is not a regression in the gate. Do not re-list findings here — the audit's own output is the inventory, and `b.gkz` is the source of truth for what is expected versus new.
