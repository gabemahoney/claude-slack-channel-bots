...
// ---------------------------------------------------------------------------
// b.en2 Epic 4: non-dialog wrapper-migration assertions
// Each case proves (a) outage-class typed error raises the flag via the
// wrapper AND (b) postSpawnFailureToChannel is NOT invoked for that error.
// ---------------------------------------------------------------------------

describe('b.en2 Epic 4: non-dialog wrapper-migration assertions', () => {
  const DEV_CHANNELS_PANE_E4 = readFileSync(
    join(import.meta.dir, 'fixtures', 'dev-channels-pane-2.1.120.txt'),
    'utf-8',
  )

  // Site #1 — reconnectMcp sendKeys
  test('#1 reconnectMcp: ErrSystemInstallDisappeared raises ad-unreachable, pSFTC not called', async () => {
    installStub({ sendKeysError: new ErrSystemInstallDisappeared('send-keys', '/bin/ad') })
    const { web, calls } = makeMockWeb()
    const cfg = makeRoutingConfig({ routes: { C: { cwd: '/x' } } })
    const result = await reconnectMcp('C', web as unknown as Parameters<typeof reconnectMcp>[1], cfg)
  ...
  test('#8 waitForWaitingAndReconnect: ErrSystemInstallDisappeared raises ad-unreachable, pSFTC not called', async () => {
    installStub({ statusError: new ErrSystemInstallDisappeared('status', '/bin/ad') })
    _setWaitForWaitingTimeoutMs(100)
    const { web, calls } = makeMockWeb()
    const cfg = makeRoutingConfig({ routes: { C: { cwd: '/x' } } })
    const result = await waitForWaitingAndReconnect('C', cfg, web as unknown as Parameters<typeof waitForWaitingAndReconnect>[2])
    expect(result).toBe(false)
    expect(getOutageFlags('C').has('ad-unreachable')).toBe(true)
    expect(calls.length).toBe(0)
  })

  // Site #9 — tryKill kill (via spawnForRoute kill+delete path)
  // tryKill swallows all errors; withOutageDetection still raises the flag.
  // Subsequent delete and spawn succeed — onset emission is the assertion.
  test('#9 tryKill: ErrSystemInstallDisappeared raises ad-unreachable, pSFTC not called', async () => {
    installStub({
      killError: new ErrSystemInstallDisappeared('kill', '/bin/ad'),
      spawnQueue: [
        cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision()),
        cannedOk<import('agent-director').SpawnResult>({ claude_instance_id: 'cscb_C' }),
      ],
      readPaneResults: [{ pane: DEV_CHANNELS_PANE_E4 }, { pane: 'clear' }, { pane: 'clear' }],
      getResult: cannedGetResult({ claude_instance_id: 'cscb_C', state: 'missing' }),
    })
    const { web, calls } = makeMockWeb()
    const cfg = makeRoutingConfig({ routes: { C: { cwd: '/x' } }, resume_enabled: false })
    await spawnForRoute('C', { cwd: '/x' }, cfg, web as any)
    const onset = outageEmissions.find(e => e.channelId === 'C' && e.text.includes('agent-director unreachable'))
    expect(onset).toBeDefined()
    expect(calls.length).toBe(0)
  })

  // Site #10 — tryDelete delete
  test('#10 tryDelete: ErrSystemInstallDisappeared raises ad-unreachable, pSFTC not called', async () => {
    installStub({
      spawnQueue: [cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision())],
      getResult: cannedGetResult({ claude_instance_id: 'cscb_C', state: 'missing' }),
      deleteError: new ErrSystemInstallDisappeared('delete', '/bin/ad'),
    })
    const { web, calls } = makeMockWeb()
    const cfg = makeRoutingConfig({ routes: { C: { cwd: '/x' } }, resume_enabled: false })
    const result = await spawnForRoute('C', { cwd: '/x' }, cfg, web as any)
    expect(result.action).toBe('failed')
    expect(getOutageFlags('C').has('ad-unreachable')).toBe(true)
    expect(calls.length).toBe(0)
  })

  // Site #11 — spawnForRoute initial spawn (ErrSystemInstallDisappeared)
  test('#11 initial spawn: ErrSystemInstallDisappeared raises ad-unreachable, pSFTC not called', async () => {
    installStub({ spawnError: new ErrSystemInstallDisappeared('spawn', '/bin/ad') })
    const { web, calls } = makeMockWeb()
    const cfg = makeRoutingConfig({ routes: { C: { cwd: '/x' } } })
    const result = await spawnForRoute('C', { cwd: '/x' }, cfg, web as any)
    expect(result.action).toBe('failed')
  ...
  // Site #11 variant — initial spawn ErrCwdNotFound → cwd-unreachable
  test('#11b initial spawn: ErrCwdNotFound raises cwd-unreachable, pSFTC not called', async () => {
    installStub({ spawnError: new ErrCwdNotFound('spawn', 'ErrCwdNotFound', 'cwd not found') })
    const { web, calls } = makeMockWeb()
    const cfg = makeRoutingConfig({ routes: { C: { cwd: '/missing' } } })
    const result = await spawnForRoute('C', { cwd: '/missing' }, cfg, web as any)
    expect(result.action).toBe('failed')
    expect(getOutageFlags('C').has('cwd-unreachable')).toBe(true)
    expect(calls.length).toBe(0)
  })

  // Site #12 — collision-handling get
  test('#12 collision get: ErrSystemInstallDisappeared raises ad-unreachable, pSFTC not called', async () => {
    installStub({
      spawnQueue: [cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision())],
      getError: new ErrSystemInstallDisappeared('get', '/bin/ad'),
    })
    const { web, calls } = makeMockWeb()
    const cfg = makeRoutingConfig({ routes: { C: { cwd: '/x' } } })
    const result = await spawnForRoute('C', { cwd: '/x' }, cfg, web as any)
  ...
  // Site #13 — retry-spawn after ErrSpawnNotFound
  test('#13 retry-spawn after ErrSpawnNotFound: ErrSystemInstallDisappeared raises ad-unreachable, pSFTC not called', async () => {
    installStub({
      spawnQueue: [
        cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision()),
        cannedErr<import('agent-director').SpawnResult>(new ErrSystemInstallDisappeared('spawn', '/bin/ad')),
      ],
      getError: errSpawnNotFound(),
    })
    const { web, calls } = makeMockWeb()
  ...
  // Site #14 — fresh spawn after kill+delete (resume_enabled=false path)
  test('#14 fresh spawn after kill+delete (resume_enabled=false): ErrSystemInstallDisappeared raises flag, pSFTC not called', async () => {
    installStub({
      spawnQueue: [
        cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision()),
        cannedErr<import('agent-director').SpawnResult>(new ErrSystemInstallDisappeared('spawn', '/bin/ad')),
  ...
  // Site #15 — resume
  test('#15 resume: ErrSystemInstallDisappeared raises ad-unreachable, pSFTC not called', async () => {
    installStub({
      spawnQueue: [cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision())],
      getResult: cannedGetResult({ claude_instance_id: 'cscb_C', state: 'ended' }),
      resumeError: new ErrSystemInstallDisappeared('resume', '/bin/ad'),
    })
    const { web, calls } = makeMockWeb()
    const cfg = makeRoutingConfig({ routes: { C: { cwd: '/x' } } })
    const result = await spawnForRoute('C', { cwd: '/x' }, cfg, web as any)
  ...
  // Site #16 — fresh spawn after ErrNoSessionId delete
  test('#16 fresh spawn after ErrNoSessionId delete: ErrSystemInstallDisappeared raises flag, pSFTC not called', async () => {
    installStub({
      spawnQueue: [
        cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision()),
        cannedErr<import('agent-director').SpawnResult>(new ErrSystemInstallDisappeared('spawn', '/bin/ad')),
  ...
  // Site #17 — fresh spawn after ErrSpawnNotResumable kill+delete
  test('#17 fresh spawn after ErrSpawnNotResumable kill+delete: ErrSystemInstallDisappeared raises flag, pSFTC not called', async () => {
    installStub({
      spawnQueue: [
        cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision()),
        cannedErr<import('agent-director').SpawnResult>(new ErrSystemInstallDisappeared('spawn', '/bin/ad')),
  ...
  // Site #18 — reconcileInstanceIds per-orphan delete
  test('#18 reconcileInstanceIds per-orphan delete: ErrSystemInstallDisappeared raises ad-unreachable, pSFTC not called', async () => {
    installStub({
      listResult: {
        spawns: [
          cannedListRow({
    ...
    expect(r.deleted).toBe(0)
    expect(getOutageFlags('C0AMDDZEHCY').has('ad-unreachable')).toBe(true)
  })
})

// ---------------------------------------------------------------------------
// b.en2 Epic 4: dialog wrapper-migration assertions
// Six cases — one per dialog AD call (sites #2-#7) — each proves:
//   (a) ErrSystemInstallDisappeared raises ad-unreachable via the wrapper
//   (b) the dialog function returns normally (error swallowed by poll loop)
// Uses dialog poll seams to keep each test well under 1 s.
// ---------------------------------------------------------------------------

describe('b.en2 Epic 4: dialog wrapper-migration assertions', () => {
  const DEV_CHANNELS_PANE_D = readFileSync(
    join(import.meta.dir, 'fixtures', 'dev-channels-pane-2.1.120.txt'),
    'utf-8',
  )
  const TRUST_PANE = `  > ${TRUST_DIALOG_NEEDLE}`

  // Site #2 — approveDevChannelsDialog detection readPane
  test('#2 approveDevChannelsDialog detection readPane: ErrSystemInstallDisappeared raises flag, dialog returns normally', async () => {
    _setDialogPollTimeoutMs(50)
    installStub({ readPaneError: new ErrSystemInstallDisappeared('read-pane', '/bin/ad') })
    await expect(approveDevChannelsDialog('C', undefined, false, undefined)).resolves.toBeUndefined()
    expect(getOutageFlags('C').has('ad-unreachable')).toBe(true)
  })

  // Site #3 — approveDevChannelsDialog sendKeys (Enter key)
  test('#3 approveDevChannelsDialog sendKeys: ErrSystemInstallDisappeared raises flag, dialog returns normally', async () => {
    _setDialogPollTimeoutMs(50)
    const stub = installStub({
      sendKeysError: new ErrSystemInstallDisappeared('send-keys', '/bin/ad'),
      readPaneResults: [{ pane: DEV_CHANNELS_PANE_D }],
    })
    void stub
    await expect(approveDevChannelsDialog('C', undefined, false, undefined)).resolves.toBeUndefined()
    expect(getOutageFlags('C').has('ad-unreachable')).toBe(true)
  ...
  // Site #4 — approveDevChannelsDialog confirmation readPane
  test('#4 approveDevChannelsDialog confirmation readPane: ErrSystemInstallDisappeared raises flag, dialog returns normally', async () => {
    _setDialogPollTimeoutMs(150)
    const stub = installStub({})
    let rpIdx = 0
    stub.readPane = async (_p: import('agent-director').ReadPaneParams): Promise<import('agent-director').ReadPaneResult> => {
      rpIdx++
      if (rpIdx === 1) return { pane: DEV_CHANNELS_PANE_D }
      throw new ErrSystemInstallDisappeared('read-pane', '/bin/ad')
    }
    await expect(approveDevChannelsDialog('C', undefined, false, undefined)).resolves.toBeUndefined()
    expect(getOutageFlags('C').has('ad-unreachable')).toBe(true)
  })

  // Site #5 — approveTrustFolderDialog detection readPane
  test('#5 approveTrustFolderDialog detection readPane: ErrSystemInstallDisappeared raises flag, dialog returns normally', async () => {
    _setTrustDialogPollTimeoutMs(50)
    installStub({ readPaneError: new ErrSystemInstallDisappeared('read-pane', '/bin/ad') })
    await expect(approveTrustFolderDialog('C', undefined, false, undefined)).resolves.toBeUndefined()
    expect(getOutageFlags('C').has('ad-unreachable')).toBe(true)
  })

  // Site #6 — approveTrustFolderDialog sendKeys (Enter key)
  test('#6 approveTrustFolderDialog sendKeys: ErrSystemInstallDisappeared raises flag, dialog returns normally', async () => {
    _setTrustDialogPollTimeoutMs(50)
    const stub = installStub({
      sendKeysError: new ErrSystemInstallDisappeared('send-keys', '/bin/ad'),
  ...
  // Site #7 — approveTrustFolderDialog confirmation readPane
  test('#7 approveTrustFolderDialog confirmation readPane: ErrSystemInstallDisappeared raises flag, dialog returns normally', async () => {
    _setTrustDialogPollTimeoutMs(150)
    const stub = installStub({})
    let trpIdx = 0
    stub.readPane = async (_p: import('agent-director').ReadPaneParams): Promise<import('agent-director').ReadPaneResult> => {
      trpIdx++
      if (trpIdx === 1) return { pane: TRUST_PANE }
      throw new ErrSystemInstallDisappeared('read-pane', '/bin/ad')
    }
    await expect(approveTrustFolderDialog('C', undefined, false, undefined)).resolves.toBeUndefined()
    expect(getOutageFlags('C').has('ad-unreachable')).toBe(true)
...
// ---------------------------------------------------------------------------
// b.en2 Epic 4: spawn/resume success-clear assertions
// For each spawn/resume wrapped site: pre-raise all three outage flags,
// drive the site to success, assert all flags cleared and exactly one
// all-clear emission names all three classes.
// ---------------------------------------------------------------------------

describe('b.en2 Epic 4: spawn/resume success-clear assertions', () => {
  const DEV_CHANNELS_PANE_SC = readFileSync(
    join(import.meta.dir, 'fixtures', 'dev-channels-pane-2.1.120.txt'),
    'utf-8',
  )

  /** Assert all-clear emitted naming all three classes; flags empty. */
  function assertAllClear(channelId: string): void {
    expect(getOutageFlags(channelId).size).toBe(0)
    const allClear = outageEmissions.filter(e => e.channelId === channelId && e.text.includes('All clear'))
  ...
  // Site #11 — initial spawn success-clear
  test('#11 initial spawn success clears all three flags', async () => {
    preSetAllFlags('C')
    installStub({
      readPaneResults: [{ pane: DEV_CHANNELS_PANE_SC }, { pane: 'clear' }, { pane: 'clear' }],
    })
    _setDialogPollTimeoutMs(50)
    const cfg = makeRoutingConfig({ routes: { C: { cwd: '/test/cwd' } } })
    const result = await spawnForRoute('C', { cwd: '/test/cwd' }, cfg)
    expect(result.action).toBe('spawned')
    assertAllClear('C')
  })

  // Site #13 — retry-spawn after ErrSpawnNotFound success-clear
  test('#13 retry-spawn after ErrSpawnNotFound success clears all three flags', async () => {
    preSetAllFlags('C')
    installStub({
      spawnQueue: [
        cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision()),
  ...
  // Site #14 — fresh spawn after kill+delete (resume_enabled=false) success-clear
  test('#14 fresh spawn after kill+delete (resume_enabled=false) success clears all three flags', async () => {
    preSetAllFlags('C')
    installStub({
      spawnQueue: [
        cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision()),
    ...
    const cfg = makeRoutingConfig({ routes: { C: { cwd: '/test/cwd' } }, resume_enabled: false })
    const result = await spawnForRoute('C', { cwd: '/test/cwd' }, cfg)
    expect(result.action).toBe('spawned')
    assertAllClear('C')
  })

  // Site #15 — resume success-clear
  test('#15 resume success clears all three flags', async () => {
    preSetAllFlags('C')
    installStub({
      spawnQueue: [cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision())],
      getResult: cannedGetResult({ claude_instance_id: 'cscb_C', state: 'ended' }),
  ...
  // Site #16 — fresh spawn after ErrNoSessionId delete success-clear
  test('#16 fresh spawn after ErrNoSessionId delete success clears all three flags', async () => {
    preSetAllFlags('C')
    installStub({
      spawnQueue: [
        cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision()),
  ...
  // Site #17 — fresh spawn after ErrSpawnNotResumable kill+delete success-clear
  test('#17 fresh spawn after ErrSpawnNotResumable kill+delete success clears all three flags', async () => {
    preSetAllFlags('C')
    installStub({
      spawnQueue: [
        cannedErr<import('agent-director').SpawnResult>(errInstanceIdCollision()),
