  ...
  const timer = setTimeout(async () => {
    ...
    try {
      ...
      if (alive) {
        ...
        // do NOT recordFailure here. SR-25.1 / single counting site: when the
        // reconnect escalates (escalate-dead), the adapter re-enters via
        // scheduleRestart → launchSession — counting will happen there if the
        // relaunch fails. Counting here would double-count that attempt.
        return
      }
